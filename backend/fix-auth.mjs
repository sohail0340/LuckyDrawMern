...
const authFile = path.join(__dirname, 'src', 'controllers', 'authController.ts');

const cleanContent = `import bcrypt from "bcryptjs";
...
export async function register(req: Request, res: Response) {
  try {
    ...
    const isAdminEmail = isEmail && identifier === adminEmail;
...
export async function login(req: Request, res: Response) {
  try {
    ...
    const isAdminEmail = Boolean(user.email) && user.email.toLowerCase() === getAdminEmail();
...
fs.writeFileSync(authFile, cleanContent, 'utf-8');
console.log('✓ authController.ts has been cleaned and rebuilt');
